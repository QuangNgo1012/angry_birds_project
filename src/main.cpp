...
Box createBox(float x, float y, float width, float height, float density,
...
void render(sf::RenderWindow &w, float x, float y, float density,
            float friction) {
  w.clear();
  // Define box2D circle shape for objects
  b2CircleShape circle;
  circle.m_p = b2Vec2(0, 0);
  circle.m_radius = 0.3;

  // Define fixture and combine shape
  b2FixtureDef fixtureDef;
  fixtureDef.density = density;
  fixtureDef.friction = friction;
  ...
  // Define new object
  Bird *bird = new Terence(body, 0.3);

  //  Draw sprite
  b2Body *drawBody = bird->GetBody();
  b2Vec2 pos = drawBody->GetPosition();
  sf::Sprite sprite = bird->GetSprite();
  sprite.setPosition(pos.x * PPM, WINDOW_HEIGHT - pos.y * PPM);
  sprite.setRotation(-1 * drawBody->GetAngle() * DEG_PER_RAD);
  /*
  sf::RectangleShape rect;

  // For the correct Y coordinate of our drawable rect, we must substract
  // from WINDOW_HEIGHT because SFML uses OpenGL coordinate system where X
  // is right, Y is down while Box2D uses traditional X is right, Y is up
  ...
  // For the rect to be rotated in the crrect direction, we have to
  // multiply by -1
...
int main() {
  // Setup SFML window
  sf::RenderWindow w(sf::VideoMode(WINDOW_WIDTH, WINDOW_HEIGHT),
                     "SFML + Box2D");
  ...
  for (int i = 0; i < 307; i++) {
    // Starting positions are randomly generated: x between 50 and 550, y
    // between 70 and 550
    auto &&box = createBox(50 + (std::rand() % (550 - 50 + 1)),
                           70 + (std::rand() % (550 - 70 + 1)), 24, 24, 1.f,
                           0.7f, sf::Color::White);
    boxes.push_back(box);
  }
  ...
  */
  ...
  while (w.isOpen()) {
    ...
    // Render everything
    render(w, 50 + (std::rand() % (550 - 50 + 1)),
           70 + (std::rand() % (550 - 70 + 1)), 24, 24);
  }
  return 0;
}